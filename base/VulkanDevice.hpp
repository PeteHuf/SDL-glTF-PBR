...
namespace vks
{
	...
	struct VulkanDevice
	{
		...
		*/
		void createBuffer(SDL_GPUBufferUsageFlags usageFlags, /*VkMemoryPropertyFlags memoryPropertyFlags,*/ Uint32 size, SDL_GPUBuffer* *buffer/*, VkDeviceMemory *memory,*/ /*void *data = nullptr, Uint32 *actualBufferSize = nullptr*/)
		{
			...
			*buffer = SDL_CreateGPUBuffer(logicalDevice, &create_info);
			...
			// assert(data == nullptr); // only transfer buffers can map
		...
		* @return VK_SUCCESS if buffer handle and memory have been created and (optionally passed) data has been copied
		*/
		void createTransferBuffer(SDL_GPUTransferBufferUsage usageFlags, /*VkMemoryPropertyFlags memoryPropertyFlags,*/ Uint32 size, SDL_GPUTransferBuffer* *buffer, /*VkDeviceMemory *memory,*/ void *data = nullptr/*, Uint32 *actualBufferSize = nullptr*/)
		{
			// Create the buffer handle
			...
			*buffer = SDL_CreateGPUTransferBuffer(logicalDevice, &create_info);
			...
			if (data != nullptr)
			{
				void *mapped = SDL_MapGPUTransferBuffer(logicalDevice, *buffer, false);
				memcpy(mapped, data, size);
				// If host coherency hasn't been requested, do a manual flush to make writes visible
				// if ((memoryPropertyFlags & VK_MEMORY_PROPERTY_HOST_COHERENT_BIT) == 0)
				// {
				...
				// 	vkFlushMappedMemoryRanges(logicalDevice, 1, &mappedRange);
				// }
				SDL_UnmapGPUTransferBuffer(logicalDevice, *buffer);
