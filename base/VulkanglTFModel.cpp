...
namespace vkglTF
{
	...
	void Model::loadTextures(tinygltf::Model &gltfModel, vks::VulkanDevice *device, vks::DummyQueue* transferQueue)
	{
		...
		for (tinygltf::Texture &tex : gltfModel.textures) {
			...
			if (tex.sampler == -1) {
				...
				textureSampler.magFilter = SDL_GPU_FILTER_LINEAR;
				textureSampler.minFilter = SDL_GPU_FILTER_LINEAR;
				textureSampler.addressModeU = SDL_GPU_SAMPLERADDRESSMODE_REPEAT;
				textureSampler.addressModeV = SDL_GPU_SAMPLERADDRESSMODE_REPEAT;
				textureSampler.addressModeW = SDL_GPU_SAMPLERADDRESSMODE_REPEAT;
			} else {
	...
	void Model::loadFromFile(std::string filename, vks::VulkanDevice* device, vks::DummyQueue* transferQueue, float scale)
	{
		...
		Uint32 vertexBufferSize = static_cast<Uint32>(vertexCount * sizeof(Vertex));
		Uint32 indexBufferSize = static_cast<Uint32>(indexCount * sizeof(uint32_t));
	...
	void Model::drawNode(Node *node, /*SDL_GPUCommandBuffer* commandBuffer*/ SDL_GPURenderPass* renderPass)
	{
		if (node->mesh) {
		...
		for (auto& child : node->children) {
			drawNode(child, /*commandBuffer*/ renderPass);
		}
	}

	void Model::draw(/*SDL_GPUCommandBuffer* commandBuffer*/ SDL_GPURenderPass* renderPass)
	{
		// const VkDeviceSize offsets[1] = { 0 };
		// vkCmdBindVertexBuffers(commandBuffer, 0, 1, &vertices.buffer, offsets);
		// vkCmdBindIndexBuffer(commandBuffer, indices.buffer, 0, VK_INDEX_TYPE_UINT32);

		SDL_GPUBufferBinding buffBinding{ .buffer = vertices.buffer, .offset = 0 };
		SDL_GPUBufferBinding indBinding{ .buffer = indices.buffer, .offset = 0 };
		...
		for (auto& node : nodes) {
			drawNode(node, /*commandBuffer*/ renderPass);
		}
	}
